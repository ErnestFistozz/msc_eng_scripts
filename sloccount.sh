...
do
    ...
    then 
        ...
        echo "$scc_loc, $scc_complexity" >> $repo_dir/$scripts_dir/$codecov/$filename-scc.csv
        echo "done"
    fi
done < <(tail -n +1 $repo_dir/$scripts_dir/$codecov/$filename.csv)
